...
(function () {
  ...
  /* ======================================================================
     FONDO AMBIENTE DE "SELECTED WORKS"
     Un video de fondo propio por pestana (Sound / Audiovisual), detras
     del titulo, el selector y la grilla. Arranca solo con la pestana
     por defecto, recien cuando esa zona entra en pantalla; el video de
     la otra pestana no se descarga hasta que se la elige por primera
     vez. Al cambiar de pestana los dos se cruzan (ver la transicion de
     opacity en .works-bg__video), y el que queda atras se pausa.
     ====================================================================== */

  (function () {
    var worksBg = document.querySelector(".works-bg");
    if (!worksBg) return;

    var bgVideos = {};
    Array.prototype.slice.call(worksBg.querySelectorAll("[data-portfolio-bg]")).forEach(function (v) {
      bgVideos[v.dataset.portfolioBg] = v;
    });

    var active = "sound"; /* coincide con la pestana que arranca elegida */
    var inView = false;
    ...
    function pauseBg(name) {
      var v = bgVideos[name];
      if (v && !v.paused) v.pause();
    }

    Object.keys(bgVideos).forEach(function (name) {
      bgVideos[name].classList.toggle("is-active", name === active);
    });

    Array.prototype.slice.call(document.querySelectorAll(".portfolio-tab")).forEach(function (tab) {
      tab.addEventListener("click", function () {
        var name = tab.dataset.portfolioTarget;
        if (!bgVideos[name] || name === active) return;
        pauseBg(active);
        bgVideos[active].classList.remove("is-active");
        active = name;
        bgVideos[active].classList.add("is-active");
        playBg(active);
      });
    });

    if (!reduceMotion.matches && "IntersectionObserver" in window) {
      var worksBgObserver = new IntersectionObserver(function (entries) {
        entries.forEach(function (entry) {
          inView = entry.isIntersecting;
          if (inView) playBg(active);
          else pauseBg(active);
        });
      }, { threshold: 0.15 });
      afterLoad(function () { worksBgObserver.observe(worksBg); });
    }

    document.addEventListener("visibilitychange", function () {
      if (document.hidden) pauseBg(active);
      else if (inView) playBg(active);
    });
  })();
})();
